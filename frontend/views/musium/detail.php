    ...
    <div class="row clearfix">
        ...
        <div class="col-md-10 column">
            <ul class="breadcrumb">
                ...
                <li>
                    <a href="<?=Url::to(['musium/index'])?>">旅行故事</a>
                </li>
                <li class="active">
                    <?php echo $musiumdetail[0]['musiumname']  ?>
                </li>
            </ul>
            <p class="text-center text-info lead" style="margin-top: 50px;font-size: 25px">
                <strong><?=$musiumdetail[0]['musiumname']   ?></strong>
            </p>
            <dl class="dl-horizontal" style="font-size: 15px;font-family: 'Microsoft YaHei'">
                ...
                <dd>
                    <strong><?=$musiumdetail[0]['musiumcontent']   ?></strong>
                </dd>
                <dt style="margin-top: 15px;">
                    发布人:
                </dt>
                <dd>
                    <strong><?=$musiumdetail[0]['username'] ?></strong>
                    <?php if(yii::$app->user->identity->getId()!=$musiumdetail[0]['uid']){ ?>
                ...
                <dd>
                    <strong><?=$musiumdetail[0]['email']   ?></strong>
                </dd>
                <dt>
                    手机号:
                </dt>
                <dd>
                    <strong><?=$musiumdetail[0]['uphone']   ?></strong>
                </dd>
                <dt>
                    发布时间:
                </dt>
                <dd>
                    <strong><?=date('Y-m-d H:i:s',$musiumdetail[0]['createtime'])   ?></strong>
                </dd>
                <dt>
                    状态:
                </dt>
                <dd>
                    <strong><?=$musiumdetail[0]['status']   ?></strong>
                </dd>
                <?php if(yii::$app->user->identity->getId()==$musiumdetail[0]['uid']){ ?>
                    <dt>
                        操作:
                    ...
                    <dd>
                        <button class="btn btn-primary" onclick="finshmusium('<?=$musiumdetail[0]['musiumid']?>','<?=$musiumdetail[0]['status']?>');">结贴</button>
                        <button class="btn btn-primary" onclick="deletemusium('<?=$musiumdetail[0]['musiumid']?>');">删除</button>
                    </dd>
                <?php }?>
            </dl>
            <div class="carousel slide"  id="carousel-612878">
                <div class="carousel-inner">
                    <div class="item active" >
                        <img  alt="" class="center-block" style="height: 500px;width: 1600px" src="<?=$musiumdetail[0]['musiumimage'] ?>" />
            ...
            <div class="tabbable" id="tabs-721112" style="margin-top: 50px">
                ...
                <div class="tab-content">
                            ...
                            foreach ($content as $row){  ?>
                                <div class="col-md-12" style="border: 1px solid #ddd">
                                    ...
                                    <div class="col-md-8" style="margin-top: 50px;font-size: 20px">
                                        <p>
                                            <em><?=$row['musiumcommenttext']?></em>
                                        </p>
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#showmorecontent" onclick="showmoreres(<?=$row['musiumcommentid']?>)">查看更多回复</button><br />
                                    </div>
                                    <div class="col-md-2" style="margin-top: 50px">
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#rescontent" onclick="editres(<?=$row['musiumcommentid']?>)">回复</button><br />
                    ...
                    <div class="tab-pane active" id="panel-988930">
                        <?php $form = ActiveForm::begin(['method' => "post",'action'=>Url::to(['musium/addcontent']),'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
                        <div class="col-md-12">
                        ...
                        <input type="text" name="musiumid" style="display: none" value="<?=$musiumdetail[0]['musiumid']?>">
...
<div class="modal fade" id="rescontent" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php $form = ActiveForm::begin(['method' => "post",'action'=>Url::to(['musium/addres']),'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="text" class="form-control" name="musiumid" value="<?=Yii::$app->request->get('musiumid')?>" style="display: none">

                <input type="text" class="form-control" name="musiumcommontid" id="musiumcommontid" style="display: none">
                <input type="text" class="form-control" name="uid" id="uid" value="<?=Yii::$app->user->getId()?>" style="display: none">
                <input type="text" class="form-control" name="musiumcommontres" id="musiumcommontres" required>
            </div>
...
<script>
    ...
    function showmoreres(musiumcommontid) {
        var musiumcommontid=musiumcommontid;
        var formdata=new FormData();
        formdata.append('musiumcommontid',musiumcommontid);
        $.ajax({
            url:'<?=Url::to(['musium/showcontentres']) ?>',
            type:'POST',
            cache:false,
            data:formdata,
            dataType:'json',
...
//                         div+=">";
                        div+="</span>";
                        div+="<span>"+objval['musiumcommentrestext']+"</span>";
    ...
    function finshmusium(musiumid,status) {
        if(status=='已结贴'){
            alert("您无须进行该操作");
            return false;
        }
        var formdata=new FormData();
        formdata.append('musiumid',musiumid);
        $.ajax({
            url: '<?=Url::to(['musium/finishmusium']) ?>',
            type: 'POST',
            cache: false,
            data: formdata,
            dataType: 'json',
    ...
    function deletemusium(musium) {
        var formdata=new FormData();
        formdata.append('musiumid',musium);
        $.ajax({
            url: '<?=Url::to(['musium/deletemusium']) ?>',
            type: 'POST',
            cache: false,
            data: formdata,
            dataType: 'json',
            ...
            success: function (data){
                if(data){
                    alert("删除成功");
                    window.location.href="<?=Url::to(['musium/index'])?>";
                }
            }
        })
    }
    function returnchat() {
        window.open("<?=Url::to(['chatpoint/index','tochatid'=>$musiumdetail[0]['uid']])?>");
    }
</script>
