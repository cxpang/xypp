    ...
    <div class="row clearfix">
        ...
        <div class="col-md-10 column">
            <ul class="breadcrumb">
                ...
                <li>
                    <a href="<?=Url::to(['compet/index'])?>">旅行故事</a>
                </li>
                <li class="active">
                    <?php echo $competdetail[0]['competname']  ?>
                </li>
            </ul>
            <p class="text-center text-info lead" style="margin-top: 50px;font-size: 25px">
                <strong><?=$competdetail[0]['competname']   ?></strong>
            </p>
            <dl class="dl-horizontal" style="font-size: 15px;font-family: 'Microsoft YaHei'">
                ...
                <dd style="color: #9A0000">
                    <strong><?=$competdetail[0]['compettime']   ?></strong>
                </dd>
                <dt>
                    内容:
                </dt>
                <dd>
                    <strong><?=$competdetail[0]['competcontent']   ?></strong>
                </dd>
                <dt style="margin-top: 15px;">
                    发布人:
                </dt>
                <dd>
                    <strong><?=$competdetail[0]['username'] ?></strong>
                    <?php if(yii::$app->user->identity->getId()!=$competdetail[0]['uid']){ ?>
                ...
                <dd>
                    <strong><?=$competdetail[0]['email']   ?></strong>
                </dd>
                <dt>
                    手机号:
                </dt>
                <dd>
                    <strong><?=$competdetail[0]['uphone']   ?></strong>
                </dd>
                <dt>
                    发布时间:
                </dt>
                <dd>
                    <strong><?=date('Y-m-d H:i:s',$competdetail[0]['createtime'])   ?></strong>
                </dd>
                <dt>
                    状态:
                </dt>
                <dd>
                    <strong><?=$competdetail[0]['status']   ?></strong>
                </dd>
                <?php if(yii::$app->user->identity->getId()==$competdetail[0]['uid']){ ?>
                    <dt>
                        操作:
                    ...
                    <dd>
                        <button class="btn btn-primary" onclick="finshcompet('<?=$competdetail[0]['competid']?>','<?=$competdetail[0]['status']?>');">结贴</button>
                        <button class="btn btn-primary" onclick="deletecompet('<?=$competdetail[0]['competid']?>');">删除</button>
                    </dd>
                <?php }?>
            </dl>
            <div class="carousel slide"  id="carousel-612878">
                <div class="carousel-inner">
                    <div class="item active" >
                        <img  alt="" class="center-block" style="height: 500px;width: 1600px" src="<?=$competdetail[0]['competimage'] ?>" />
            ...
            <div class="tabbable" id="tabs-721112" style="margin-top: 50px">
                ...
                <div class="tab-content">
                            ...
                            foreach ($content as $row){  ?>
                                <div class="col-md-12" style="border: 1px solid #ddd">
                                    ...
                                    <div class="col-md-8" style="margin-top: 50px;font-size: 20px">
                                        <p>
                                            <em><?=$row['competcommenttext']?></em>
                                        </p>
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#showmorecontent" onclick="showmoreres(<?=$row['competcommentid']?>)">查看更多回复</button><br />
                                    </div>
                                    <div class="col-md-2" style="margin-top: 50px">
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#rescontent" onclick="editres(<?=$row['competcommentid']?>)">回复</button><br />
                    ...
                    <div class="tab-pane active" id="panel-988930">
                        <?php $form = ActiveForm::begin(['method' => "post",'action'=>Url::to(['compet/addcontent']),'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
                        <div class="col-md-12">
                        ...
                        <input type="text" name="competid" style="display: none" value="<?=$competdetail[0]['competid']?>">
...
<div class="modal fade" id="rescontent" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php $form = ActiveForm::begin(['method' => "post",'action'=>Url::to(['compet/addres']),'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="text" class="form-control" name="competid" value="<?=Yii::$app->request->get('competid')?>" style="display: none">

                <input type="text" class="form-control" name="competcommontid" id="competcommontid" style="display: none">
                <input type="text" class="form-control" name="uid" id="uid" value="<?=Yii::$app->user->getId()?>" style="display: none">
                <input type="text" class="form-control" name="competcommontres" id="competcommontres" required>
            </div>
...
<script>
    ...
    function showmoreres(competcommontid) {
        var competcommontid=competcommontid;
        var formdata=new FormData();
        formdata.append('competcommontid',competcommontid);
        $.ajax({
            url:'<?=Url::to(['compet/showcontentres']) ?>',
            type:'POST',
            cache:false,
            data:formdata,
            dataType:'json',
...
//                         div+=">";
                        div+="</span>";
                        div+="<span>"+objval['competcommentrestext']+"</span>";
    ...
    function finshcompet(competid,status) {
        if(status=='已结贴'){
            alert("您无须进行该操作");
            return false;
        }
        var formdata=new FormData();
        formdata.append('competid',competid);
        $.ajax({
            url: '<?=Url::to(['compet/finishcompet']) ?>',
            type: 'POST',
            cache: false,
            data: formdata,
            dataType: 'json',
    ...
    function deletecompet(compet) {
        var formdata=new FormData();
        formdata.append('competid',compet);
        $.ajax({
            url: '<?=Url::to(['compet/deletecompet']) ?>',
            type: 'POST',
            cache: false,
            data: formdata,
            dataType: 'json',
            ...
            success: function (data){
                if(data){
                    alert("删除成功");
                    window.location.href="<?=Url::to(['compet/index'])?>";
                }
            }
        })
    }
    function returnchat() {
        window.open("<?=Url::to(['chatpoint/index','tochatid'=>$competdetail[0]['uid']])?>");
    }
</script>
