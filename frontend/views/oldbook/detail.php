    ...
    <div class="row clearfix">
        ...
        <div class="col-md-10 column">
            <ul class="breadcrumb">
                ...
                <li>
                    <a href="<?=Url::to(['oldbook/index'])?>">旅行故事</a>
                </li>
                <li class="active">
                    <?php echo $oldbookdetail[0]['oldbookname']  ?>
                </li>
            </ul>
            <p class="text-center text-info lead" style="margin-top: 50px;font-size: 25px">
                <strong><?=$oldbookdetail[0]['oldbookname']   ?></strong>
            </p>
            <dl class="dl-horizontal" style="font-size: 15px;font-family: 'Microsoft YaHei'">
                ...
                <dd style="color: #9A0000">
                    <strong><?=$oldbookdetail[0]['oldbookprice']   ?>RMB</strong>
                </dd>
                ...
                <dd>
                    <strong><?=$oldbookdetail[0]['oldbookintro']   ?></strong>
                </dd>
                ...
                <dd>
                    <strong><?=$oldbookdetail[0]['email']   ?></strong>
                </dd>
                <dt>
                    手机号:
                </dt>
                <dd>
                    <strong><?=$oldbookdetail[0]['uphone']   ?></strong>
                </dd>
                <dt>
                    发布时间:
                </dt>
                <dd>
                    <strong><?=date('Y-m-d H:i:s',$oldbookdetail[0]['createtime'])   ?></strong>
                </dd>
                <dt>
                    状态:
                </dt>
                <dd>
                    <strong><?=$oldbookdetail[0]['status']   ?></strong>
                </dd>
                <?php if(yii::$app->user->identity->getId()==$oldbookdetail[0]['uid']){ ?>
                    <dt>
                        操作:
                    ...
                    <dd>
                        <button class="btn btn-primary" onclick="finsholdbook('<?=$oldbookdetail[0]['oldbookid']?>','<?=$oldbookdetail[0]['status']?>');">结贴</button>
                        <button class="btn btn-primary" onclick="deleteoldbook('<?=$oldbookdetail[0]['oldbookid']?>');">删除</button>
                    </dd>
                <?php }?>
            </dl>
            <div class="carousel slide"  id="carousel-612878">
                <div class="carousel-inner">
                    <div class="item active" >
                        <img  alt="" class="center-block" style="height: 500px;width: 1600px" src="<?=$oldbookdetail[0]['oldbookimage'] ?>" />
            ...
            <div class="tabbable" id="tabs-721112" style="margin-top: 50px">
                ...
                <div class="tab-content">
                            ...
                            foreach ($content as $row){  ?>
                                <div class="col-md-12" style="border: 1px solid #ddd">
                                    ...
                                    <div class="col-md-8" style="margin-top: 50px;font-size: 20px">
                                        <p>
                                            <em><?=$row['oldbookcommenttext']?></em>
                                        </p>
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#showmorecontent" onclick="showmoreres(<?=$row['oldbookcommentid']?>)">查看更多回复</button><br />
                                    </div>
                                    <div class="col-md-2" style="margin-top: 50px">
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#rescontent" onclick="editres(<?=$row['oldbookcommentid']?>)">回复</button><br />
                    ...
                    <div class="tab-pane active" id="panel-988930">
                        <?php $form = ActiveForm::begin(['method' => "post",'action'=>Url::to(['oldbook/addcontent']),'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
                        <div class="col-md-12">
                        ...
                        <input type="text" name="oldbookid" style="display: none" value="<?=$oldbookdetail[0]['oldbookid']?>">
...
<div class="modal fade" id="rescontent" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php $form = ActiveForm::begin(['method' => "post",'action'=>Url::to(['oldbook/addres']),'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="text" class="form-control" name="oldbookid" value="<?=Yii::$app->request->get('oldbookid')?>" style="display: none">

                <input type="text" class="form-control" name="oldbookcommontid" id="oldbookcommontid" style="display: none">
                <input type="text" class="form-control" name="uid" id="uid" value="<?=Yii::$app->user->getId()?>" style="display: none">
                <input type="text" class="form-control" name="oldbookcommontres" id="oldbookcommontres" required>
            </div>
...
<script>
    ...
    function showmoreres(oldbookcommontid) {
        var oldbookcommontid=oldbookcommontid;
        var formdata=new FormData();
        formdata.append('oldbookcommontid',oldbookcommontid);
        $.ajax({
            url:'<?=Url::to(['oldbook/showcontentres']) ?>',
            type:'POST',
            cache:false,
            data:formdata,
            dataType:'json',
...
//                         div+=">";
                        div+="</span>";
                        div+="<span>"+objval['oldbookcommentrestext']+"</span>";
    ...
    function finsholdbook(oldbookid,status) {
        if(status=='已结贴'){
            alert("您无须进行该操作");
            return false;
        }
        var formdata=new FormData();
        formdata.append('oldbookid',oldbookid);
        $.ajax({
            url: '<?=Url::to(['oldbook/finisholdbook']) ?>',
            type: 'POST',
            cache: false,
            data: formdata,
            dataType: 'json',
    ...
    function deleteoldbook(oldbook) {
        var formdata=new FormData();
        formdata.append('oldbookid',oldbook);
        $.ajax({
            url: '<?=Url::to(['oldbook/deleteoldbook']) ?>',
            type: 'POST',
            cache: false,
            data: formdata,
            dataType: 'json',
            ...
            success: function (data){
                if(data){
                    alert("删除成功");
                    window.location.href="<?=Url::to(['oldbook/index'])?>";
                }
            }
        })
    }
