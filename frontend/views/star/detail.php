    ...
    <div class="row clearfix">
        ...
        <div class="col-md-10 column">
            <ul class="breadcrumb">
                ...
                <li>
                    <a href="<?=Url::to(['star/index'])?>">旅行故事</a>
                </li>
                <li class="active">
                    <?php echo $stardetail[0]['starname']  ?>
                </li>
            </ul>
            <p class="text-center text-info lead" style="margin-top: 50px;font-size: 25px">
                <strong><?=$stardetail[0]['starname']   ?></strong>
            </p>
            <dl class="dl-horizontal" style="font-size: 15px;font-family: 'Microsoft YaHei'">
                ...
                <dd style="color: #9A0000">
                    <strong><?=$stardetail[0]['starprice']   ?>RMB</strong>
                </dd>
                <dt>
                    时间:
                </dt>
                <dd style="color: #9A0000">
                    <strong><?=$stardetail[0]['startime']   ?></strong>
                </dd>
                <dt>
                    内容:
                </dt>
                ...
                <dd>
                    <strong><?=$stardetail[0]['email']   ?></strong>
                </dd>
                <dt>
                    手机号:
                </dt>
                <dd>
                    <strong><?=$stardetail[0]['uphone']   ?></strong>
                </dd>
                <dt>
                    发布时间:
                </dt>
                <dd>
                    <strong><?=date('Y-m-d H:i:s',$stardetail[0]['createtime'])   ?></strong>
                </dd>
                <dt>
                    状态:
                </dt>
                <dd>
                    <strong><?=$stardetail[0]['status']   ?></strong>
                </dd>
                <?php if(yii::$app->user->identity->getId()==$stardetail[0]['uid']){ ?>
                    <dt>
                        操作:
                    ...
                    <dd>
                        <button class="btn btn-primary" onclick="finshstar('<?=$stardetail[0]['starid']?>','<?=$stardetail[0]['status']?>');">结贴</button>
                        <button class="btn btn-primary" onclick="deletestar('<?=$stardetail[0]['starid']?>');">删除</button>
                    </dd>
                <?php }?>
            </dl>
            <div class="carousel slide"  id="carousel-612878">
                <div class="carousel-inner">
                    <div class="item active" >
                        <img  alt="" class="center-block" style="height: 500px;width: 1600px" src="<?=$stardetail[0]['starimage'] ?>" />
            ...
            <div class="tabbable" id="tabs-721112" style="margin-top: 50px">
                ...
                <div class="tab-content">
                            ...
                            foreach ($content as $row){  ?>
                                <div class="col-md-12" style="border: 1px solid #ddd">
                                    ...
                                    <div class="col-md-8" style="margin-top: 50px;font-size: 20px">
                                        <p>
                                            <em><?=$row['starcommenttext']?></em>
                                        </p>
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#showmorecontent" onclick="showmoreres(<?=$row['starcommentid']?>)">查看更多回复</button><br />
                                    </div>
                                    <div class="col-md-2" style="margin-top: 50px">
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#rescontent" onclick="editres(<?=$row['starcommentid']?>)">回复</button><br />
                    ...
                    <div class="tab-pane active" id="panel-988930">
                        <?php $form = ActiveForm::begin(['method' => "post",'action'=>Url::to(['star/addcontent']),'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
                        <div class="col-md-12">
                        ...
                        <input type="text" name="starid" style="display: none" value="<?=$stardetail[0]['starid']?>">
...
<div class="modal fade" id="rescontent" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php $form = ActiveForm::begin(['method' => "post",'action'=>Url::to(['star/addres']),'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="text" class="form-control" name="starid" value="<?=Yii::$app->request->get('starid')?>" style="display: none">

                <input type="text" class="form-control" name="starcommontid" id="starcommontid" style="display: none">
                <input type="text" class="form-control" name="uid" id="uid" value="<?=Yii::$app->user->getId()?>" style="display: none">
                <input type="text" class="form-control" name="starcommontres" id="starcommontres" required>
            </div>
...
<script>
    ...
    function showmoreres(starcommontid) {
        var starcommontid=starcommontid;
        var formdata=new FormData();
        formdata.append('starcommontid',starcommontid);
        $.ajax({
            url:'<?=Url::to(['star/showcontentres']) ?>',
            type:'POST',
            cache:false,
            data:formdata,
            dataType:'json',
...
//                         div+=">";
                        div+="</span>";
                        div+="<span>"+objval['starcommentrestext']+"</span>";
    ...
    function finshstar(starid,status) {
        if(status=='已结贴'){
            alert("您无须进行该操作");
            return false;
        }
    ...
    function deletestar(star) {
        var formdata=new FormData();
        formdata.append('starid',star);
        $.ajax({
            url: '<?=Url::to(['star/deletestar']) ?>',
            type: 'POST',
            cache: false,
            data: formdata,
            dataType: 'json',
            ...
            success: function (data){
                if(data){
                    alert("删除成功");
                   window.location.href="<?=Url::to(['star/index'])?>";
                }
            }
        })
    }
